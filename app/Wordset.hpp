...
public: 
	...
	// finds the next empty bucket if collision.
	int findEmptyBucket(int & full_bucket, std::unique_ptr<std::string[]>& the_bucket, int the_cap );

private:
	// You may declare private functions and member variables here.
	unsigned int capacity;
	unsigned int sz;
	std::unique_ptr<std::string[]> buckets;
};
